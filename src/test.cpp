...
{
	unsigned num = N > 2 ? atoll(args[2]) : 1000000, tmp, count = 0;
	...
	{
		fprintf(fp, "insert into user %u %s %s %u\n", i, (string("user") + to_string(tmp = random_age(generator))).c_str(), (string("email") + to_string(i)).c_str(), random_age(generator));
		if(tmp == 10) id_set.insert(i);
	}
	for(unsigned i = 0 ; i < num ; i++)
	{
	...
	//fprintf(fp, "select count(*) from user join like on id = id2 where name = 10\n");
	fprintf(fp, ".exit\n");
	printf("ans : %u\nnumber of id : %u\n", count, id_set.size());
	return 0;
}